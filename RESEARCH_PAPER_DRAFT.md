...
1. **การดึงข้อมูลจากแหล่งเปิดสาธารณะ:** คัดกรองรายชื่อพืชพรรณท้องถิ่นที่พบในมหาวิทยาลัยราชภัฏวไลยอลงกรณ์ ในพระบรมราชูปถัมภ์ จำนวน 140 ชนิดพันธุ์ นำมาสืบค้นภาพผ่าน GBIF API
2. **การจำกัดปริมาณข้อมูล (Capping):** ทำการจำกัดจำนวนภาพไว้ที่ 100 ภาพต่อคลาส เพื่อหลีกเลี่ยงปัญหาความไม่สมดุลของข้อมูล (Class Imbalance) ส่งผลให้ได้ชุดข้อมูลเริ่มต้นรวมทั้งสิ้น 14,000 ภาพ จากพืชทั้งหมด 140 ชนิด
...
4. **การเพิ่มข้อมูลภาพด้วยโปรแกรม (Data Augmentation):** เพื่อป้องกันการเกิดโอเวอร์ฟิตติ้ง (Overfitting) และช่วยให้แบบจำลองเรียนรู้คุณลักษณะที่ไม่ขึ้นกับทิศทางและสภาพแสง ได้ประยุกต์ใช้เทคนิคการสุ่มหมุนภาพ (Random Rotation) $\pm 15$ องศา, การสุ่มซูมภาพ (Random Zoom) ในช่วง $\pm 10\%$, การสุ่มปรับค่าความสว่าง (Random Brightness) ในช่วง $\pm 10\%$ และการพลิกภาพตามแนวนอน (Horizontal Flip)
...
ในการวิจัยรอบเริ่มต้นนี้ ได้ทำการแช่แข็งน้ำหนักของโมเดลฐาน (`base_model.trainable = False`) และฝึกสอนเฉพาะชั้นจำแนกประเภท (Fully Connected Head) ที่เชื่อมต่อเพิ่ม ได้แก่ Global Average Pooling 2D, Dropout (0.5), Dense (256) และ Dense Classifier (140 คลาส พร้อม Softmax)

ในส่วนการปรับแต่งเชิงลึก (Fine-Tuning) ภายหลังการตรึงน้ำหนักช่วงแรก ได้ทำการปลดล็อกเลเยอร์ระดับสูง (Top-level Layers) ของแต่ละโมเดลฐาน ได้แก่ โมดูลบล็อกคอนโวลูชันสุดท้าย (Last Convolutional Block) ของสถาปัตยกรรมแต่ละแบบ (เช่น ปลดล็อกบล็อกเลเยอร์ตั้งแต่ระดับ 100 ขึ้นไปสำหรับ MobileNetV2 และ EfficientNetB0) เพื่อปรับจูนค่าพารามิเตอร์ให้เหมาะสมกับสัณฐานวิทยาของพืชท้องถิ่น

### 3.3 พารามิเตอร์และสภาวะแวดล้อมในการทดลอง
*   **Optimizer:** Adam พร้อมอัตราการเรียนรู้เริ่มต้น (Learning Rate) เท่ากับ $10^{-4}$ ในขั้นตอนการตรึงน้ำหนัก และปรับลดลงเหลือ $10^{-5}$ ในช่วงการปรับแต่งเชิงลึก (Fine-Tuning) เพื่อป้องกันไม่ให้น้ำหนักที่เรียนรู้มาแล้วถูกทำลายอย่างรวดเร็ว
...
4. **การวิเคราะห์ข้อผิดพลาดของแบบจำลอง (Error Analysis):** เมื่อวิเคราะห์ความผิดพลาดเชิงลึกพบว่า แบบจำลองมักเกิดความสับสนสูงในกลุ่มพืชที่มีลักษณะใบทางพฤกษศาสตร์ใกล้เคียงกันมาก เช่น ระหว่างสกุลไทรด้วยกัน (เช่น ไทรทอง *Ficus microcarpa L. f.* กับ ไทรย้อยใบแหลม *Ficus benjamina L.*) เนื่องจากมีความคล้ายคลึงกันทางกายภาพสูง (Fine-grained categories) และเกิดความสับสนในพืชที่มีโครงสร้างกลีบดอกและสีดอกใกล้เคียงกัน เช่น พุดซ้อน (*Gardenia jasminoides*) และพุดจีบ (*Tabernaemontana divaricata*) ซึ่งสะท้อนข้อจำกัดของแบบจำลองที่ต้องอาศัยกลไกการโฟกัสจุดสนใจเพิ่มเติมในระดับละเอียด (Fine-grained attention mechanisms) เพื่อแยกแยะความแตกต่างเหล่านี้ในอนาคต

![รูปที่ 1: กราฟเปรียบเทียบค่าความแม่นยำ (Accuracy) และค่าความสูญเสีย (Loss) ตลอดการฝึกสอนรวมการปรับแต่งเชิงลึก (Fine-Tuning) ของทั้ง 3 สถาปัตยกรรมโมเดล (มีกำกับแกนแสดงค่า Epochs และ Metrics ไว้อย่างชัดเจน)](data/dataset/plant_model_comparison_merged_plot.png)
